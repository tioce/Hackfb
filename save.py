...
def yahoo():
    ...
    try:
        am = requests.get('https://graph.facebook.com/' + axt + '?access_token=' + toket)
	op = json.loads(am.text)
	print "\033[32mNama : {}".format(op["name"])
    except KeyError:
        print "Blum Berteman / Tidak Di temukan !"
        raw_input('\n\x1b[1;91m[ \x1b[1;97mKembali \x1b[1;91m]')
        menu_yahoo()
	
    ab = requests.get('https://graph.facebook.com/' + axt + '?fields=friends.limit(5000)&access_token=' + toket)
    ...
    def scan(arg):
        toket = open('login.txt', 'r').read()
	auh = requests.get('https://graph.facebook.com/' + arg + '?access_token=' + toket)
